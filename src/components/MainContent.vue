        ...
        <ul class="p-4">
            ...
            <li v-for="task in tasks" :key="task" class="flex items-center p-3 border-b border-gray-300">
                <div class="flex items-center w-full">
                    <button v-if="task.isCompleted" @click="updateTask(task._id, task.isCompleted)" class="mr-2" >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        </svg>
                    </button>
                    <button v-else @click="updateTask(task._id, task.isCompleted)" class="mr-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
function updateTask(_id, isCompleted) {
    const req = {
        _id: _id,
...
function deleteTask(_id) {
    axios.delete(url+'/api/delete'+'/'+_id)
        .then(() => {
            readTodos()
        }
    )
